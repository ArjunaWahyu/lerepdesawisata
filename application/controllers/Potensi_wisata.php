...
class potensi_wisata extends CI_Controller {
        public function __construct() {
            parent::__construct();
            $this->load->model('setting_model');
            $this->load->model('categorymedia_model');
            $this->load->model('potensi_wisata_model');
        ...
        {           
         ...
         public function page($offset=0, $offset=0, $year = null, $month = null, $day = null){
            ...
            $row = $this->navigations_model->get_by_name('Potensi Wisata');            

            // $jml = $this->potensi_wisata_model->get_by_all_type('Y');
            
            $config['base_url'] = base_url().'potensi_wisata/page/';
            ...
            $data['dataq'] = $this->potensi_wisata_model->get_by_list('Y',$config['per_page'], $offset, '1');
            ...
            $this->load->view('frontend/potensi_wisata',$data);

        }

        public function read($url=null,$offset=0, $year = null, $month = null, $day = null){
            $row                = $this->potensi_wisata_model->get_by_url($url);
            $galery             = $this->media_model->get_by_idcat($row->id);
            $contents           = $this->potensi_wisata_model->get_by_related('Y',100,0);
            $setting            = $this->setting_model->get_by_published('Y');
            $data['about']      = $setting->dest;
            ...
            $update             = array(
                 ...
                 $this->potensi_wisata_model->update($row->id, $update);
            ...
            $this->load->view('frontend/single_potensi',$data);     
        }
