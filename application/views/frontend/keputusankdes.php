...
class keputusankdes extends CI_Controller {
        ...
        public function index()
         ...
         public function page($offset=0, $year = null, $month = null, $day = null){
            ...
            // $row = $this->keputusan_model->get_by_list();            

            $jml = $this->keputusan_model->get_by_all_type('Y', 2);
            
            $config['base_url'] = base_url().'keputusan/page/';
            ...
            $data['offset'] = $offset;
             ...
             $data['data'] = $this->keputusan_model->get_by_list('Y',$config['per_page'], $offset, '2');
            
            $setting = $this->keputusan_model->get_by_published('Y');
            $data['about'] = $setting->dest;
            $data['metatittle']   = $setting->metatittle;
            $data['metadest']   = $setting->metadest;
            $data['metakey']   = $setting->metakey;
            $data['header']   = 'frontend/header';
            $data['footer']   = 'frontend/footer';
            $data['navigation']   = 'frontend/navigation';
            $data['type']   = $setting->name;
            ...
            $this->load->view('frontend/keputusan',$data);

        }

        public function read($url=null, $year = null, $month = null, $day = null){
            $row                = $this->keputusan_model->get_by_url($url);
            $contents           = $this->keputusan_model->get_by_related('Y',4,0,$row->type);
            ...
            $update = array(
                 ...
                 $this->keputusan_model->update($row->id, $update);
